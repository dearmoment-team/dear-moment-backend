...
class UpdateProductUseCaseTest : BehaviorSpec({
    ...
    // 실제 테스트 대상 UseCase
    val useCase = UpdateProductUseCaseImpl(
    ...
    // 더미 파일
    val mainImageFileTest = MockMultipartFile(
        "mainImageFile",
        "new_main.jpg",
        "image/jpeg",
        "new main content".toByteArray()
    )
    val subImageFileForUpload1 = MockMultipartFile(
        "subImageFiles",
        "sub3.jpg",
        "image/jpeg",
        "sub3-content".toByteArray()
    )
    val subImageFileForUpload2 = MockMultipartFile(
        "subImageFiles",
        "sub4.jpg",
        "image/jpeg",
        "sub4-content".toByteArray()
    )
    val additionalImageFileTest = MockMultipartFile(
    ...
    val dummyExistingSubImage1 = Image(
        imageId = 201L,
        userId = dummyUserId,
        parId = "",
    ...
    val dummyExistingSubImage2 = Image(
        imageId = 202L,
        userId = dummyUserId,
        parId = "",
    ...
    val dummyExistingSubImage3 = Image(
        imageId = 203L,
        userId = dummyUserId,
        parId = "",
    ...
    Given("updateProduct 메서드") {
        ...
        When("정상 요청 시") {
            ...
            every { productPersistencePort.save(any(), any()) } returns existingProduct.copy(
                title = "Updated Title",
                description = "Updated Description",
            ...
            val result = useCase.updateProduct(
                userId = dummyUserId,
                productId = updateRequest.productId,
                rawRequest = updateRequest,
        ...
        When("서브 이미지를 1개만 교체하고, 나머지 3개는 그대로 유지할 때") {
            val oneChangeRequest = baseUpdateRequest.copy(
                subImagesFinal = listOf(
        ...
        When("서브 이미지를 2개만 교체하고, 나머지 2개는 그대로 유지할 때") {
            val twoChangeRequest = baseUpdateRequest.copy(
                subImagesFinal = listOf(
                    SubImageFinalRequest(UpdateSubImageAction.KEEP, 0, dummyExistingSubImage1.imageId),
                    SubImageFinalRequest(UpdateSubImageAction.KEEP, 1, dummyExistingSubImage2.imageId),
                    SubImageFinalRequest(UpdateSubImageAction.UPLOAD, 2, null),
        ...
        When("서브 이미지를 3개만 교체하고, 나머지 1개는 그대로 유지할 때") {
            val threeChangeRequest = baseUpdateRequest.copy(
                subImagesFinal = listOf(
                    SubImageFinalRequest(UpdateSubImageAction.KEEP, 0, dummyExistingSubImage1.imageId),
        ...
        When("서브 이미지를 4개 모두 교체할 때") {
            val allChangeRequest = baseUpdateRequest.copy(
                subImagesFinal = listOf(
        ...
        When("옵션 수정 및 신규 옵션 추가 시나리오") {
            val partnerShopRequest = UpdatePartnerShopRequest(
                category = "DRESS",
                name = "Partner1",
                link = "http://partner1.com"
            )
            val updatedOption1 = UpdateProductOptionRequest(
                optionId = 1L,
            ...
            val newOption = UpdateProductOptionRequest(
                optionId = null,
                name = "New Option",
                optionType = "PACKAGE",
            ...
            val expectedOptionResponses = optionRequestList.map {
                ProductOptionResponse.fromDomain(UpdateProductOptionRequest.toDomain(it, 999L))
            }

            every { getProductPort.findById(999L) } returns existingProduct
            every { imageHandler.updateMainImage(any(), any(), any()) } returns dummyNewMainImage
            every { imageHandler.processSubImagesPartial(any(), any(), any(), any()) } returns listOf(
                dummyExistingSubImage,
                dummyExistingSubImage,
                dummyExistingSubImage,
                dummyExistingSubImage,
            )
            every { imageHandler.processAdditionalImagesFinal(any(), any(), any(), any()) } returns listOf(
