...
class ProductOptionEntityTest : StringSpec({
    ...
    "ProductOptionEntity는 도메인 모델에서 올바르게 변환되어야 한다" {
        ...
        // 이미지 매핑 검증: ProductEntity의 images는 ImageEntity 리스트이므로, 각 ImageEntity의 fileName을 추출하여 도메인 Product의 images의 fileName 리스트와 비교
    ...
    "ProductOptionEntity는 도메인 모델로 올바르게 변환되어야 한다" {
        ...
        // 이미지 필드는 미리 생성된 ImageEntity 객체로 구성 (url도 함께 설정)
        val imageEntity1 =
            ImageEntity.from(
                Image(
                    imageId = 0L,
