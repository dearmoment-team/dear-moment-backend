...
class ProductEntityTest : StringSpec({
    ...
    "ProductEntity는 도메인 모델에서 올바르게 변환되어야 한다" {
        ...
        // 이미지 매핑 검증: productEntity.images는 ImageEntity 리스트이므로 fileName만 추출하여 비교
    ...
    "ProductEntity는 도메인 모델로 올바르게 변환되어야 한다" {
        ...
        // 엔티티의 BaseTime 필드가 null이므로 도메인의 createdAt, updatedAt도 null이어야 함
        product.createdAt shouldBe null
        product.updatedAt shouldBe null

        // toDomain()에서 images는 ImageEntity.fileName을 추출하여 String 리스트로 변환
